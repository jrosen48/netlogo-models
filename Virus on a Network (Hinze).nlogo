...
to setup
  ...
  ask n-of number-infected turtles
    [ become-infected ]
  ask n-of number-immune turtles with [not infected?]
    [ become-immune ]
...
to setup-nodes
  ...
  [
    ...
    become-susceptible
    ; set virus-check-timer random 1
  ]
end

to setup-spatially-clustered-network
  let num-links (average-connections * number-of-people) / 2
...
to become-infected  ;; turtle procedure
  set infected? true
  set immune? false
  set count-down? (recovery-days + 1)
  set color red
end
...
to recover
  ask turtles with [count-down? = 0]
    [become-susceptible]
end

to spread-virus
  ask turtles with [infected?]
    [ ask link-neighbors with [not (infected? or immune?)]
...
; Adapted by Joshua Rosenberg and Paul Hinze (2016)
@#$#@#$#@
GRAPHICS-WINDOW
257
10
...
number-immune
0
number-of-people - number-infected
6
1
1
NIL
...
"" ""
PENS
"infected" 1 0 -2674135 true "" "plot count turtles with [infected?] / number-of-people"
"susceptible" 1 0 -10899396 true "" "plot count turtles with [not infected?] / number-of-people"
